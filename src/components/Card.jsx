...
function Card({ nftId,nftImage, nftName,collectionImage,CollectionName,price,onClick,collectionId ,userId}) {
  ...
  return (
    <div className="w-[17rem] border-[1px] border-gray flex flex-col items-center p-[1rem] h-[26rem] rounded-[1.5rem] gap-[1rem]">
      ...
      <div className="w-full flex flex-col justify-between gap-[1rem]">
        ...
        <div className="flex items-center">
          <div className="flex flex-1  items-center justify-start font-semibold gap-[0.5rem]">
            <AvatarImage onClick={()=> collectionId? navigate(`/collection/${collectionId}`) :  navigate(`/profile/${userId}`)} collectionImage={collectionImage || avatar} />
            <div role="button" onClick={()=> collectionId? navigate(`/collection/${collectionId}`) :  navigate(`/profile/${userId}`)}>{CollectionName || "Creator"}</div>
          </div>
      ...
      <div className="w-[15rem] flex justify-center">
        {price? <Button name="Buy" onClick={(e) => navigate(`/asset/${nftId}`)} /> : <Button secondary={true} name="Not on sale" onClick={(e) => navigate(`/asset/${nftId}`)}  />}
